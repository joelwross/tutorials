<?php 
$title = 'Essential Git';
$subtitle = 'How to manage your files like a pro';
include '../header.php';
?>
...
<dl>
    ...
    <dd>Pulls all commits made since the last pull from another repository, and attempts to merge those changes into your current files. If the pulled commits involve changes to the same lines that you've already changed locally, this process will produce a <span class="term">merge conflict</span> that you will have to resolve.</dd>
</dl>
...
<h2>Creating and Cloning a Repository</h2>
...
<p>Start by <a href="https://help.github.com/articles/create-a-repo/">creating a new repo on GitHub</a>. Since this is just a test, you can name it whatever you want, and you can make it public (GitHub charges for private repos).</p>
...
<p>The <code>mkdir</code> or "make directory" command creates a new subdirectory within the current directory, and the name of that new subdirectory follows the command (in this case "code"). The <code>cd</code> or "change directory" command changes the current directory to whatever path follows the command. In this case we changed into the new subdirectory we created during the previous command.</p>
...
<p>Sometimes you want git to ignore particular directories or files in your project. For example, if you use a Mac and you tend to organize your files in the finder by dragging them around, the Mac will create a hidden file in that folder named <code>.DS_Store</code> to track the position of all the icons. You don't want to add a file like that to your git repository, as it will likely differ from machine to machine. If you add this sort of file, and you work on your code from multiple machines (or work in a team) you will end up with lots of merge conflicts.</p>

<p>You can tell git to ignore files like this by creating a special file in your project directory named <code>.gitignore</code>. If git sees a file with this name in your project directory, it will ignore directories or files that are listed in the file. The file uses a very simple format: each line should list a directory or file path to ignore. For example:</p>
...
<ol>
    <li>Create a simple <code>index.html</code> file in the repository directory.</li>
...
<p>The <code>--no-edit</code> option tells git to use the default revert message instead of prompting you for one, and <code>HEAD</code> tells git to revert the most recent commit.</p>
...
<ol>
    ...
    <li>Each developer then creates a new branch off of <code>develop</code> for each new feature or set of related features. Code in these branches can be incomplete and unstable, and is largely private to the developer or developers working on those features.</li>
    <li>Once the feature is complete and tested, the feature developer creates a <strong>pull request</strong> on GitHub, asking to merge all commits from the feature branch into the <code>develop</code> branch. The changes are reviewed by the dev lead or a committee, and if approved, the commits are then merged into the <code>develop</code> branch.</li>
...
<p>In this course, most of your work will be done individually, so the full implementation of git flow is probably a bit overkill. But using a <code>develop</code> branch is still a good idea, so let's practice that now.</p>

<p>In your repository directory, execute this command to create a new <code>develop</code> branch off of <code>master</code> and switch to it:</p>

<pre><code class="language-bash">
$ git checkout -b develop
</code></pre>

<p>To verify that you are on the <code>develop</code> branch, execute <code>git status</code> and ensure that it says <code>On branch develop</code>. You can also execute <code>git branch</code> to see all branches with the current one prefixed with a star (and in green on most systems).</p>
...
<p>Look at your <code>index.html</code> file again and notice that the change you made is now gone. But don't worry---it's still there, but it's on the <code>develop</code> branch, not the <code>master</code> branch. Switch back to the <code>develop</code> branch using this command, and notice that your change returns:</p>

<pre><code class="language-bash">
$ git checkout develop
</code></pre>

<p>To merge the changes you committed to <code>develop</code> into <code>master</code>, use these commands:</p>
...
<p>Unless there's a conflict, git should just merge all the commits made to <code>develop</code> into <code>master</code> without any errors, and your <code>master</code> branch should now contain the changes you made over in the <code>develop</code> branch.</p>

<h2>GitHub Pull Requests</h2>

<p>Although git lets you merge commits from one branch to another, most professional teams use a <span class="term">pull request</span> when merging feature work into the <code>develop</code> or <code>master</code> branch. A pull request facilitates a process called <span class="term">code review</span>, where other developers review your changes, make comments, and ask you to correct anything they find problematic. Once your changes are approved, the reviewers will then merge your commits into the target branch.</p>
...
<ol>
    <li>Switch back to your <code>develop</code> branch (<code>git checkout develop</code>)</li>
    <li>Make another change to your <code>index.html</code> file, add it to the staging area, commit it, and push to GitHub. Note that when you push it may generate an error saying <code>the current branch has no upstream branch</code>, but it will also show you the exact command to run to set the upstream branch during the push. Run that command now.</li>
    <li>Navigate to your repository on GitHub's web site, and choose the <strong>New Pull Request</strong> button. </li>
    <li>Choose <code>master</code> as the base branch, and <code>develop</code> as the compare branch. GitHub should automatically determine the set of commits to merge and let you know that the branches can be automatically merged.</li>
...
<p>To merge the commits into the <code>master</code> branch, go back to the <strong>Conversation</strong> tab and choose the <strong>Merge pull request</strong> button.</p>
...
<p>This will pull all new commits from the server that you don't yet have in your local development repo.</p>
...
<li>Only the files and commits in the <code>gh-pages</code> branch are visible on the web. All commits in other branches (<code>develop</code>, <code>master</code>, etc.) are not visible on the web. This allows you to work on your site with others before publishing those changes to the web.</li>
